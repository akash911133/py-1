...
def test_resolve_variable_reference(variables):
    ...
    assert resolved_value == {"resolved_value": "v1.2.3", "variable_name": "nginx_version"}
...
def test_extract_resource_info(mock_open, terraform_config, variables):
    ...
    resource_info = extract_resource_info("fake_terraform.tf", "fake_chart.yml", variables=variables)
...
def test_process_module_files(mock_open, terraform_config, variables):
    ...
    module_list = process_module_files(module_files, variables=variables)
